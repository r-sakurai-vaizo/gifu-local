...
function Nav({view,go}:{view:View;go:(v:View)=>void}){const n:[View,string,string][]=[['home','⌂','ホーム'],['market','▦','作品'],['artists','♙','作家'],['support','♡','支援'],['creator','▣','管理']];return <nav className="bottomnav">{n.map(([v,i,l])=><button key={v} className={view===v?'active':''} onClick={()=>go(v)}><span>{i}</span>{l}</button>)}</nav>}
...
function PlatformHome({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){return <main className="pf-page pf-home">
 ...
 <nav className="pf-quick">{[['作品を買う','market','WORKS'],['体験を予約','experience','EXPERIENCE'],['作家をフォロー','artists','FOLLOW'],['作家を支援','support','PATRON'],['法人購入相談','corporate','FOR BUSINESS']].map(x=><button key={x[0]} onClick={()=>go(x[1] as View)}><b>{x[0]}</b><small>{x[2]}</small><span>›</span></button>)}</nav>
