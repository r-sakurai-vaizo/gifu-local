...
/* ---------- ART PASS バナー・ページ ---------- */
function PassBanner({go}:{go:(v:View)=>void}){return <button className="pass-banner" onClick={()=>go('pass')}><div><small>GIFU ART TICKET（構想デモ）</small><h2>美術館も、ギャラリーも、工芸の店も。<br/>1枚のチケットで、まとめて巡る。</h2><p>岐阜公園〜川原町〜司町〜柳ヶ瀬〜駅。徒歩でつながるアートの動線を、デジタルチケット1枚で。売上の一部は作り手に還元する構想です。</p><span>アートチケットを見る　›</span></div><aside><div className="pass-card-mini"><small>GIFU ART TICKET</small><b>1DAY / ¥600</b><i>▣</i><em>この画面を施設で提示（構想）</em></div></aside></button>}
function PassPage({go,set}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void}){return <main className="content pass-page"><section className="pass-hero" style={{backgroundImage:`linear-gradient(#061827a8,#061827e6),url('${homeImages.park}')`}}><small>GIFU ART TICKET</small><h1>岐阜のアートを、<br/>1枚で巡るチケット。</h1><p>岐阜公園の美術館から、川原町の工芸、柳ヶ瀬の作家スペースまで。徒歩でつながる動線を、スマホのデジタルチケットでまとめて楽しむ——その販売導線のデモです。</p><DemoNote>これは大学プロジェクトの構想デモです。実際の販売・決済は行われません。</DemoNote></section>
 <section className="pass-plans"><h2>チケットの種類(想定価格の例)</h2><div>{passPlans.map(p=><article key={p.id}><small>{p.name}</small><strong>{p.price}</strong><b>{p.period}</b><p>{p.copy}</p><button onClick={()=>{set({passPlan:p.id});go('passcheckout')}}>このチケットを選ぶ(デモ)</button></article>)}</div><p className="pass-note">1DAYは岐阜公園の2館だけで、2DAYは公立3館で、2DAY+は三甲美術館+どれか1館で「元が取れる」想定価格(例)です。</p></section>
...
function PassDone({go,s}:{go:(v:View)=>void;s:AppState}){const sel=passPlans.find(p=>p.id===s.passPlan)||passPlans[0];return <main className="content pass-page pass-done"><span className="eyebrow">GIFU ART TICKET / DEMO</span><h1 className="page-title">チケットが発行されました（デモ）</h1>
 <section className="pass-card-big"><small>GIFU ART TICKET</small><h2>{sel.name}</h2><b>{sel.period}</b><div className="pass-qr">▣</div><p>TICKET ID：GAT-DEMO-0001</p><em>施設の受付でこの画面を提示（構想）</em></section>
