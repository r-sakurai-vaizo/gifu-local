...
type View='home'|'about'|'pass'|'passcheckout'|'passdone'|'eventlist'|'walk'|'culture'|'experience'|'shop'|'shoplist'|'maker'|'spot'|'map'|'search'|'market'|'artists'|'artwork'|'support'|'corporate'|'creator';
type AppState={saved:boolean;booked:boolean;passOwned:boolean;passPlan:string;makerId:string;artworkId:string;cart:number;following:string[];supportAmount:number};
...
const officialLinks={
 ...
 ,galleryCaption:'https://www.gallerycaption.info/'
 ,yanagaseSouko:'https://yanagasesouko.com/'
 ,gomyway:'https://gomywaygallery.webnode.jp/'
 ,kenbiEvents:'https://kenbi.pref.gifu.lg.jp/events/'
 ,kenbiNagalier:'https://kenbi.pref.gifu.lg.jp/events/birdsview3/'
 ,ozekiAkari:'https://www.ozeki-lantern.co.jp/akari.html'
 ,ozekiAerofoil:'https://www.ozeki-lantern.co.jp/product-aerofoil.html'
 ,wagasaBlackYellow:'https://wagasa.shop/collections/higasa/products/%E6%97%A5%E5%82%98-%E9%BB%92-%E9%BB%92%E9%AA%A8'
 ,sunBuild:'https://ysbmkt.com/'
 ,ninale:'https://yanagaseninale.com/'
 ,gifuCityEvents:'https://www.city.gifu.lg.jp/event_calendar.html'
 ,gifuArtExhibition:'https://www.gifu-art.jp/kaisai.html'
 ,mizunooto:'https://www.gallery-mizunooto.com/gallery-mizunooto'
};
...
/* ---------- アート／工芸の購入・体験・支援プラットフォーム ---------- */
type CreatorProfile={id:string;name:string;field:string;area:string;bio:string;image:string;cover:string;fact:string;url:string;kind:'art'|'craft'|'performance'};
const creatorProfiles:CreatorProfile[]=[
 {id:'caption',name:'GALLERY CAPTION',field:'現代美術・ギャラリー',area:'岐阜市・明徳町',bio:'1985年開廊。展覧会、プロジェクト、レクチャーを通して国内外の現代美術を紹介するギャラリー。',image:homeImages.mediacosmos,cover:homeImages.wallart,fact:'公式展覧会・作家情報あり',url:officialLinks.galleryCaption,kind:'art'},
 {id:'gomyway',name:'現代美術園 / uncellule',field:'絵画・映像・オブジェ',area:'岐阜市・柳ケ瀬',bio:'柳ケ瀬のカフェ＆バーギャラリー。作品展示販売、映像、チョークアートや樹脂粘土などのワークショップを行う。',image:homeImages.souko2,cover:homeImages.starSax,fact:'展示・販売・体験は予約優先',url:officialLinks.gomyway,kind:'art'},
 {id:'nagalier',name:'岐阜県美術館 ～ながラー',field:'アートコミュニケーション',area:'岐阜市・宇佐',bio:'岐阜県美術館を起点に、人・こころ・地域をつなぐアートコミュニケーターの活動。館長の日比野克彦氏らと活動を展開する。',image:homeImages.wallart,cover:homeImages.mediacosmos,fact:'イベント・参加情報は美術館公式へ',url:officialLinks.kenbiNagalier,kind:'performance'},
 {id:'casa',name:'和傘CASA',field:'岐阜和傘・工芸',area:'岐阜市・川原町',bio:'岐阜県で唯一の和傘専門店。複数の職人が作る本格和傘を紹介・販売し、産地と後継者育成を支える。',image:homeImages.wagasa,cover:homeImages.wagasa,fact:'公式オンラインショップあり',url:officialLinks.wagasa,kind:'craft'},
 {id:'ozeki',name:'株式会社オゼキ',field:'岐阜提灯・照明',area:'岐阜市・小熊町',bio:'1891年創業。岐阜提灯と、イサム・ノグチの照明彫刻AKARIなど、和紙のあかりを岐阜で製造する。',image:homeImages.paper,cover:homeImages.craft,fact:'本社ショールームあり',url:officialLinks.ozeki,kind:'craft'},
 {id:'souko',name:'やながせ倉庫',field:'複合アート・クラフト',area:'岐阜市・柳ケ瀬',bio:'2004年に始まったアパートメント形式の貸スペース。若手クリエイターやハンドメイド作家が店舗・アトリエを営む。',image:homeImages.souko,cover:homeImages.soukoBoard,fact:'公式店舗紹介あり',url:officialLinks.yanagaseSouko,kind:'art'}
];
type Artwork={id:string;title:string;creatorId:string;category:string;price:string;availability:string;image:string;edition:string;description:string;url:string;action:string;stock?:number};
const artworks:Artwork[]=[
 {id:'wagasa-black-yellow',title:'日傘「黒（黒骨）」（イエロー）',creatorId:'casa',category:'工芸',price:'¥37,400（公式掲載時）',availability:'1点限り・最新在庫は公式確認',image:homeImages.wagasa,edition:'和傘CASA',description:'黒骨と糸かがりの色が映える岐阜和傘。公式ページでは寸法・骨数・仕様まで確認できます。',url:officialLinks.wagasaBlackYellow,action:'公式商品ページ'},
 {id:'wagasa-janome-blue',title:'蛇の目傘「水色」',creatorId:'casa',category:'工芸',price:'¥49,500（公式掲載時）',availability:'最新在庫は公式ショップで確認',image:homeImages.wagasa,edition:'仐日和',description:'和傘CASAの公式商品一覧に掲載されている岐阜和傘。価格・在庫は変動するため公式情報を優先します。',url:officialLinks.wagasa,action:'公式ショップ'},
 {id:'akari',title:'AKARI',creatorId:'ozeki',category:'照明彫刻',price:'価格・取扱店は公式確認',availability:'シリーズ・型番により異なる',image:homeImages.paper,edition:'イサム・ノグチ / オゼキ製造',description:'1951年、イサム・ノグチと岐阜提灯の出会いから生まれた照明彫刻。岐阜の伝統技術で現在も製造されています。',url:officialLinks.ozekiAkari,action:'AKARI公式ページ'},
 {id:'aerofoil',title:'AEROFOIL',creatorId:'ozeki',category:'照明・デザイン',price:'価格は公式へ問い合わせ',availability:'製品仕様・取扱は公式確認',image:homeImages.craft,edition:'Sebastian Conran design',description:'長良杉、楮和紙、竹ひごを用い、岐阜提灯の技術を現代の照明に展開したオゼキの製品シリーズ。',url:officialLinks.ozekiAerofoil,action:'製品公式ページ'},
 {id:'caption-projects',title:'GALLERY CAPTION 展覧会・プロジェクト',creatorId:'caption',category:'現代美術',price:'作品取扱は画廊へ問い合わせ',availability:'開催・取扱作家は公式確認',image:homeImages.wallart,edition:'展覧会・作品取扱',description:'岐阜市明徳町の現代美術ギャラリー。展覧会、作家、プロジェクトの最新情報を公式サイトで確認できます。',url:officialLinks.galleryCaption,action:'展覧会・作家情報'},
 {id:'gomyway-works',title:'現代美術園 オリジナル作品',creatorId:'gomyway',category:'現代美術・映像',price:'展示・販売は要問い合わせ',availability:'来場は予約優先',image:homeImages.starSax,edition:'絵画・映像・オブジェ',description:'uncelluleによる絵画、映像、衣料品、オブジェなどを展開。展示販売や作品レンタルにも対応しています。',url:officialLinks.gomyway,action:'公式案内を見る'},
 {id:'souko-shops',title:'やながせ倉庫 入居作家の作品',creatorId:'souko',category:'クラフト・雑貨',price:'価格は各店舗・現地で確認',availability:'営業日時は店舗ごとに異なる',image:homeImages.soukoBoard,edition:'店舗・アトリエ',description:'入居するクリエイターの作品や雑貨を現地で購入できます。現在の店舗は公式の店舗紹介をご確認ください。',url:officialLinks.yanagaseSouko,action:'公式店舗紹介'}
];
const creativeFields=[['現代美術','PAINT / INSTALLATION'],['写真・映像','PHOTO / FILM'],['音楽・舞台','MUSIC / PERFORMANCE'],['イラスト・出版','ILLUSTRATION / ZINE'],['デザイン','GRAPHIC / PRODUCT'],['伝統工芸','CRAFT']];
const creatorExperiences=[
 {title:'展覧会・イベントを訪ねる',creator:'GALLERY CAPTION',date:'会期・開廊日は公式確認',price:'入場条件は展覧会ごと',image:homeImages.wallart,url:officialLinks.galleryCaption},
 {title:'チョークアート・樹脂粘土など',creator:'現代美術園 / uncellule',date:'随時・予約優先',price:'料金は公式へ問い合わせ',image:homeImages.starSax,url:officialLinks.gomyway},
 {title:'アートコミュニケーション・ワークショップ',creator:'岐阜県美術館',date:'最新日程はイベント一覧へ',price:'企画ごとに異なる',image:homeImages.mediacosmos,url:officialLinks.kenbiEvents},
 {title:'岐阜和傘の専門店を訪ねる',creator:'和傘CASA / CASA stella',date:'営業・見学情報は公式確認',price:'商品・企画ごとに異なる',image:homeImages.wagasa,url:officialLinks.casaSpot}
];

/* ---------- GIFU ART TICKET (販売導線モック・決済なし) ---------- */
const passPlans=[
 {id:'1day',name:'ART TICKET 1DAY',price:'¥600',period:'最初の入館から24時間有効',copy:'岐阜公園の2館(計620円)を巡るだけで元が取れる。'},
 {id:'2day',name:'ART TICKET 2DAY',price:'¥900',period:'最初の入館から48時間有効',copy:'一泊して県美術館まで。公立3館(計960円)で元が取れる。'},
 {id:'2dayplus',name:'ART TICKET 2DAY+',price:'¥1,500',period:'最初の入館から48時間有効',copy:'三甲美術館込み。三甲+どれか1館で元が取れる想定。'}
];
...
const citySpots=[
 ...
 {name:'GALLERY CAPTION',area:'明徳町',cat:'現代美術',url:officialLinks.galleryCaption},
 {name:'アートギャラリー・カフェ 水の音',area:'新桜町',cat:'現代美術',url:officialLinks.mizunooto},
 {name:'現代美術園 / GOMYWAYGALLERY',area:'柳ケ瀬',cat:'現代美術',url:officialLinks.gomyway},
...
const makers:MakerInfo[]=[
 ...
 {id:'souko',name:'やながせ倉庫',area:'柳ヶ瀬アーケード',craft:'雑貨・イラスト・ハンドメイド等',title:'作家が集まる、自治的な「団地」',
  ...
  links:[['公式サイト・店舗紹介',officialLinks.yanagaseSouko],['場所を地図で見る',officialLinks.soukoMap]]}
];
...
const experienceItems=[
 ...
 {category:'市民アート',title:'やながせ倉庫を歩く',copy:'作家のアトリエ兼ショップが集まるビルで、作り手と直接話す。',place:'柳ヶ瀬',info:'営業は店舗ごとに異なる',image:homeImages.souko,url:officialLinks.yanagaseSouko,spot:true,makerId:'souko'},
...
const shopItems=[
 ...
 {tag:'クラフト',title:'やながせ倉庫の作家作品',maker:'入居作家',makerId:'souko',price:'現地で作家から直接',image:homeImages.souko,url:officialLinks.yanagaseSouko},
...
const eventItems:EventItem[]=[
 {title:'サンデービルヂングマーケット',date:'毎月第3日曜／偶数月第1日曜・公式確認',place:'柳ヶ瀬アーケード',image:homeImages.souko,url:officialLinks.sunBuild,kind:'regular'},
 {title:'スタープレイス柳ヶ瀬 ネオン点灯',date:'毎週土曜の夜',place:'西柳ヶ瀬・柳ヶ瀬本通り',image:homeImages.starSax,url:officialLinks.starPlace,kind:'regular'},
 {title:'柳ケ瀬日常ニナーレ',date:'開催プログラム・募集は公式情報を確認',place:'柳ヶ瀬',image:homeImages.souko2,url:officialLinks.ninale,kind:'regular'},
 {title:'第7回ぎふ美術展',date:'2026年8月8日〜23日（終了・記録）',place:'岐阜県美術館',image:homeImages.craft,url:officialLinks.gifuArtExhibition,kind:'past'},
 {title:'岐阜市の文化・芸術イベント',date:'2026年の最新日程を市公式で確認',place:'岐阜市内',image:homeImages.mediacosmos,url:officialLinks.gifuCityEvents,kind:'check'},
 {title:'水の音 展覧会',date:'最新会期をギャラリー公式で確認',place:'岐阜市新桜町',image:homeImages.wallart,url:officialLinks.mizunooto,kind:'check'},
...
/* ---------- 共通UI ---------- */
function Top({title,back,go}:{title:string;back?:boolean;go:(v:View)=>void}){return <header className="topbar"><button className="icon" onClick={()=>go('home')} aria-label="戻る">{back?'‹':'⌁'}</button><strong>{title}</strong><button className="icon" onClick={()=>go('support')} aria-label="作家を支援">♡</button></header>}
function Nav({view,go}:{view:View;go:(v:View)=>void}){const n:[View,string,string][]=[['home','⌂','ホーム'],['market','▦','作品'],['artists','♙','作家'],['pass','▣','チケット'],['creator','⚙','管理']];return <nav className="bottomnav">{n.map(([v,i,l])=><button key={v} className={view===v?'active':''} onClick={()=>go(v)}><span>{i}</span>{l}</button>)}</nav>}
function DemoNote({children}:{children:ReactNode}){return <p className="demo-note">{children}</p>}

function FollowButton({id,s,set,notice}:{id:string;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){const active=s.following.includes(id);return <button className={`pf-follow ${active?'active':''}`} onClick={()=>{set({following:active?s.following.filter(x=>x!==id):[...s.following,id]});notice(active?'フォローを解除しました。':'作家をフォローしました。')}}>{active?'✓ フォロー中':'+ フォローする'}</button>}
function ArtworkCard({item,go,set}:{item:Artwork;go:(v:View)=>void;set:(p:Partial<AppState>)=>void}){const c=creatorProfiles.find(x=>x.id===item.creatorId)!;return <article className="pf-work-card"><button onClick={()=>{set({artworkId:item.id});go('artwork')}}><div><img src={item.image} alt=""/><span>{item.category}</span></div><small>{c.name} / {c.area}</small><h3>{item.title}</h3><footer><b>{item.price}</b><em>公式情報</em></footer></button></article>}
function PlatformHome({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){return <main className="pf-page pf-home">
 <section className="pf-hero" style={{backgroundImage:`linear-gradient(90deg,#06182bf2 0%,#06182ba8 43%,#06182b20),url('/hero-art-v2.png')`}}><div><small>GIFU ARTISTS MARKET</small><h1>岐阜の表現を、<br/>暮らしと未来へ。</h1><p>作品を買う。工房を訪ねる。作家をフォローし、旅のあとも支える。<br/>アートと工芸の作り手に、価値が直接届く場所。</p><div><button onClick={()=>go('market')}>作品を探す</button><button onClick={()=>go('artists')}>作家と出会う</button></div></div><aside><span>ART</span><span>CRAFT</span><span>PHOTO</span><span>MUSIC</span><span>FILM</span><span>DESIGN</span></aside></section>
 <nav className="pf-quick">{[['作品を買う','market','WORKS'],['体験を予約','experience','EXPERIENCE'],['アートチケット','pass','ART TICKET'],['作家をフォロー','artists','FOLLOW'],['作家を支援','support','PATRON'],['法人購入相談','corporate','FOR BUSINESS']].map(x=><button key={x[0]} onClick={()=>go(x[1] as View)}><b>{x[0]}</b><small>{x[2]}</small><span>›</span></button>)}</nav>
 <PassBanner go={go}/>
 <section className="pf-section pf-fields"><header><small>EXPLORE BY FIELD</small><h2>工芸だけではない、岐阜の表現。</h2><p>いま生まれている作品を、分野を越えて探す。</p></header><div>{creativeFields.map((x,i)=><button key={x[0]} onClick={()=>go(i===5?'shoplist':'market')}><span>0{i+1}</span><b>{x[0]}</b><small>{x[1]}</small></button>)}</div></section>
 <section className="pf-section"><header><small>VERIFIED SELECTION</small><h2>公式情報から探す作品・プロジェクト</h2><p>価格・在庫・開催状況は各公式ページの最新情報をご確認ください。</p><button onClick={()=>go('market')}>すべて見る　›</button></header><div className="pf-work-grid">{artworks.map(x=><ArtworkCard key={x.id} item={x} go={go} set={set}/>)}</div></section>
 <section className="pf-dark-section"><header><small>ARTISTS IN GIFU</small><h2>作る人を知る。</h2><p>作品だけでなく、制作の背景とこれからをフォローする。</p></header><div className="pf-creator-grid">{creatorProfiles.slice(0,4).map(c=><article key={c.id}><button className="pf-creator-main" onClick={()=>{set({makerId:c.id});go('artists')}}><img src={c.image} alt=""/><span><small>{c.field}</small><b>{c.name}</b><em>{c.area}</em></span></button><FollowButton id={c.id} s={s} set={set} notice={notice}/></article>)}</div><button className="pf-more" onClick={()=>go('artists')}>作家一覧を見る　›</button></section>
 <section className="pf-section pf-exp"><header><small>MEET THE MAKERS</small><h2>展示・制作体験</h2><p>実在する会場と公式の開催情報へ。</p><button onClick={()=>go('experience')}>体験一覧　›</button></header><div>{creatorExperiences.map(x=><a key={x.title} href={x.url} target="_blank" rel="noreferrer"><img src={x.image} alt=""/><span><small>{x.creator}</small><b>{x.title}</b><em>{x.date}　{x.price}</em></span></a>)}</div></section>
 <section className="pf-relationship"><div><small>AFTER THE TRIP</small><h2>旅が終わっても、<br/>作家との関係は続く。</h2><p>保存した作家の新作や別地域での展示を確認し、帰宅後も購入・支援できます。</p><button onClick={()=>go('artists')}>フォロー中の作家を見る</button></div><div className="pf-support-card"><small>PATRON</small><h3>一度だけ、または毎月。</h3><p>制作費、展示費、工房維持へ。使い道を選んで支援できます。</p><button onClick={()=>go('support')}>支援のかたちを選ぶ　›</button></div><div className="pf-business-card"><small>FOR BUSINESS</small><h3>空間に、その土地の作品を。</h3><p>ホテル、店舗、オフィスの作品選定・複数購入・特注制作を相談。</p><button onClick={()=>go('corporate')}>法人購入を相談する　›</button></div></section>
 <section className="pf-creator-entry"><div><small>FOR CREATORS</small><h2>作品・体験・支援者を、ひとつの場所で管理。</h2><p>在庫、価格、予約枠、注文、支援金、閲覧データ、公開設定まで。</p></div><button onClick={()=>go('creator')}>作家・職人向け管理画面を見る</button></section>
 <footer className="pf-footer"><b>GIFU ART & CRAFT</b><p>岐阜の表現を、買う・体験する・支える。</p><span>公開情報は公式ページを確認して掲載。価格・在庫・日程はリンク先の最新情報を優先してください。支援・管理機能はデモです。</span></footer>
 </main>}

function MarketPage({go,set}:{go:(v:View)=>void;set:(p:Partial<AppState>)=>void}){const [field,setField]=useState('すべて');const [q,setQ]=useState('');const filtered=artworks.filter(x=>(field==='すべて'||x.category.includes(field))&&(x.title+creatorProfiles.find(c=>c.id===x.creatorId)?.name).includes(q));return <main className="pf-page pf-list-page"><header><small>VERIFIED WORKS & PROJECTS</small><h1>作品・プロジェクトを探す</h1><p>公式情報を確認できた商品、照明、現代美術の取扱・展示情報を掲載。</p></header><div className="pf-search"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="作品・施設・作り手で検索"/><div>{['すべて','現代美術','工芸','照明','映像','クラフト'].map(x=><button className={field===x?'active':''} key={x} onClick={()=>setField(x)}>{x}</button>)}</div></div><section className="pf-market-layout"><aside><b>掲載方針</b><p>商品名・掲載価格・施設情報は公式ページを参照しています。</p><p>在庫・会期・販売条件は変動するため、各詳細から公式ページをご確認ください。</p></aside><div className="pf-work-grid">{filtered.map(x=><ArtworkCard key={x.id} item={x} go={go} set={set}/>)}</div></section></main>}
function ArtistsPage({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){return <main className="pf-page pf-list-page"><header><small>VERIFIED ARTISTS & PLACES</small><h1>作り手・活動拠点を探す</h1><p>公式サイトで活動を確認できる、岐阜市の作り手、ギャラリー、アートコミュニティ。</p></header><section className="pf-artist-list">{creatorProfiles.map(c=><article key={c.id}><img src={c.cover} alt=""/><div><small>{c.field}</small><h2>{c.name}</h2><p>{c.bio}</p><span>⌖ {c.area}　｜ {c.fact}</span><div><FollowButton id={c.id} s={s} set={set} notice={notice}/><button onClick={()=>{const work=artworks.find(x=>x.creatorId===c.id);if(work){set({artworkId:work.id});go('artwork')}else window.open(c.url,'_blank')}}>作品・活動を見る</button><a href={c.url} target="_blank" rel="noreferrer">公式ページ ↗</a></div></div></article>)}</section></main>}
function ArtworkPage({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){const item=artworks.find(x=>x.id===s.artworkId)||artworks[0];const c=creatorProfiles.find(x=>x.id===item.creatorId)!;return <main className="pf-page pf-detail"><button className="pf-back" onClick={()=>go('market')}>‹ 作品・プロジェクト一覧</button><section><div className="pf-detail-image"><img src={item.image} alt=""/><span>{item.category}</span></div><article><small>{c.field} / {c.area}</small><h1>{item.title}</h1><button className="pf-artist-link" onClick={()=>go('artists')}><img src={c.image} alt=""/><span><b>{c.name}</b><small>{c.bio}</small></span></button><strong>{item.price}</strong><p>{item.description}</p><dl><div><dt>作り手・取扱</dt><dd>{item.edition}</dd></div><div><dt>在庫・開催</dt><dd>{item.availability}</dd></div><div><dt>確認元</dt><dd>公式ページ</dd></div></dl><a className="pf-buy" href={item.url} target="_blank" rel="noreferrer">{item.action}で確認する ↗</a><button className="pf-reserve" onClick={()=>go('corporate')}>法人・空間導入を相談</button><FollowButton id={c.id} s={s} set={set} notice={notice}/></article></section><section className="pf-detail-story"><div><small>VERIFIED STORY</small><h2>公式情報で知る、背景と作り手。</h2><p>{c.bio} 商品の仕様、在庫、販売価格、展覧会期は変わる場合があります。購入・来場前に公式ページの最新情報をご確認ください。</p><OfficialLink href={c.url}>{c.name} 公式情報</OfficialLink></div><aside><h3>将来の支援機能（デモ）</h3><p>作り手の参加後に、単発・月額の支援を受け付ける想定です。現在は決済されません。</p><button onClick={()=>{set({makerId:c.id});go('support')}}>支援機能のデモを見る</button></aside></section><section className="pf-related"><header><h2>関連する実在拠点</h2><p>岐阜市内でアートと工芸に触れられる公式情報。</p></header>{[[c.area,c.url,'作り手・取扱元'],['岐阜市・宇佐',officialLinks.kenbi,'岐阜県美術館'],['岐阜市・柳ケ瀬',officialLinks.yanagaseSouko,'やながせ倉庫']].map(x=><a key={x[0]+x[2]} href={x[1]} target="_blank" rel="noreferrer"><span>公式情報</span><b>{x[0]}</b><small>{x[2]} ↗</small></a>)}</section></main>}
function SupportPage({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){const [monthly,setMonthly]=useState(true);const c=creatorProfiles.find(x=>x.id===s.makerId)||creatorProfiles[0];return <main className="pf-page pf-support"><header><small>PATRON</small><h1>作家・工房を支援する</h1><p>作品を買う以外にも、制作を続けるための力になれます。</p></header><section className="pf-support-creator"><img src={c.image} alt=""/><div><small>{c.field}</small><h2>{c.name}</h2><p>{c.bio}</p></div></section><div className="pf-support-toggle"><button className={!monthly?'active':''} onClick={()=>setMonthly(false)}>一度だけ支援</button><button className={monthly?'active':''} onClick={()=>setMonthly(true)}>毎月支援</button></div><section className="pf-support-plans">{(monthly?[1000,3000,10000]:[1000,3000,5000,10000]).map((n,i)=><button className={s.supportAmount===n?'active':''} key={n} onClick={()=>set({supportAmount:n})}><small>{monthly?'月額':'今回'}</small><strong>¥{n.toLocaleString()}</strong><b>{['制作の種','つながる応援','展示を育てる','未来を共につくる'][i]}</b><p>{monthly?'活動レポートと制作のお知らせ':'制作材料・展示費・工房維持に活用'}</p></button>)}</section><label className="pf-purpose">支援の使い道<select><option>作家におまかせ</option><option>新作の制作費</option><option>展示・発表費</option><option>工房・機材の維持</option></select></label><button className="pf-support-submit" onClick={()=>notice(`${c.name}への${monthly?'月額':'単発'}支援を受け付けました（デモ）。`)}>この内容で支援する（デモ）</button><button className="pf-back-link" onClick={()=>go('artists')}>ほかの作家を探す</button></main>}
function CorporatePage({notice}:{notice:(t:string)=>void}){return <main className="pf-page pf-corporate"><header><small>FOR BUSINESS</small><h1>法人購入・空間導入のご相談</h1><p>ホテル、飲食店、オフィス、贈答品。予算と空間に合わせて作品をご提案します。</p></header><section><aside>{[['空間に合わせた作品選定','現代美術・写真・工芸を横断して提案'],['複数・継続購入','客室、店舗、周年記念などまとまった導入'],['特注・コミッション','作家との対話から、その場所のための作品を制作'],['作家への適正還元','条件と支払先を明確にし、作り手へ届ける']].map((x,i)=><article key={x[0]}><span>0{i+1}</span><h2>{x[0]}</h2><p>{x[1]}</p></article>)}</aside><form onSubmit={e=>{e.preventDefault();notice('法人購入相談を受け付けました（デモ）。')}}><label>会社・団体名<input required placeholder="株式会社〇〇"/></label><label>ご担当者名<input required placeholder="岐阜 太郎"/></label><label>メールアドレス<input required type="email" placeholder="art@example.jp"/></label><div><label>ご予算<select><option>〜50万円</option><option>50〜200万円</option><option>200万円以上</option></select></label><label>用途<select><option>オフィス・店舗</option><option>ホテル・宿泊施設</option><option>贈答・記念品</option><option>展覧会・イベント</option></select></label></div><label>ご相談内容<textarea placeholder="設置場所、数量、希望ジャンルなど"/></label><button>内容を確認する</button></form></section></main>}
function CreatorCenter({notice}:{notice:(t:string)=>void}){const [tab,setTab]=useState('dashboard');const [published,setPublished]=useState(true);const [recommended,setRecommended]=useState(true);const tabs=[['dashboard','ダッシュボード'],['works','作品・在庫'],['slots','体験枠'],['orders','注文・予約'],['supporters','支援者・売上'],['analytics','閲覧データ'],['settings','公開設定']];return <main className="pf-page pf-creator-center"><aside><header><small>CREATOR STUDIO</small><h2>作家・職人管理</h2></header>{tabs.map(x=><button className={tab===x[0]?'active':''} key={x[0]} onClick={()=>setTab(x[0])}>{x[1]}</button>)}</aside><section><header><div><small>DEMO ACCOUNT</small><h1>{tabs.find(x=>x[0]===tab)?.[1]}</h1></div><button onClick={()=>notice('変更を保存しました（デモ）。')}>変更を保存</button></header>{tab==='dashboard'&&<><div className="pf-metrics">{[['今月の売上','¥528,400','+12.6%'],['支援金','¥86,500','+8.3%'],['注文','32件','+14件'],['体験予約','18件','+5件'],['保存','256件','+18件'],['閲覧','3,842回','+23.1%']].map(x=><article key={x[0]}><small>{x[0]}</small><strong>{x[1]}</strong><span>前月比 {x[2]}</span></article>)}</div><div className="pf-admin-panels"><article><h2>今日の対応</h2>{['新しい注文 #10032','体験予約 9/18 14:00','月額支援が届きました'].map(x=><p key={x}>● {x}<button>確認</button></p>)}</article><article><h2>売上と支援の推移</h2><div className="pf-bars">{[42,58,51,76,68,90,82].map((x,i)=><i key={i} style={{height:`${x}%`}}/>)}</div></article></div></>}{tab==='works'&&<div className="pf-admin-table"><div className="pf-admin-actions"><button>＋ 作品を登録</button><input placeholder="作品名で検索"/></div>{artworks.slice(0,5).map(x=><article key={x.id}><img src={x.image} alt=""/><b>{x.title}</b><span>{x.category}</span><label>価格<input defaultValue={x.price}/></label><label>在庫<input type="number" defaultValue={x.stock}/></label><em>{x.stock?'公開中':'在庫切れ'}</em></article>)}</div>}{tab==='slots'&&<div className="pf-slot-grid"><header><button>＋ 体験を登録</button><p>9月15日 — 9月21日</p></header>{creatorExperiences.map((x,i)=><article key={x.title}><img src={x.image} alt=""/><div><b>{x.title}</b><small>{x.date}</small></div><label>定員<input type="number" defaultValue={4+i}/></label><label>料金<input defaultValue={x.price}/></label><button>予約枠を編集</button></article>)}</div>}{tab==='orders'&&<div className="pf-admin-table pf-orders">{[['#10032','作品注文','水野 葵','¥48,000','新規'],['#10031','体験予約','河合 蓮','¥4,400','確定'],['#10030','作品注文','白木 美佳','¥66,000','発送準備'],['#10029','体験予約','和傘CASA','公式料金','完了']].map(x=><article key={x[0]}><b>{x[0]}</b><span>{x[1]}</span><span>{x[2]}</span><strong>{x[3]}</strong><em>{x[4]}</em><button>詳細</button></article>)}</div>}{tab==='supporters'&&<div className="pf-admin-panels"><article><h2>支援者</h2>{[['山田 太郎','月額 ¥3,000'],['鈴木 彩香','月額 ¥5,000'],['株式会社岐阜商会','月額 ¥30,000'],['伊藤 誠','単発 ¥3,000']].map(x=><p key={x[0]}><b>{x[0]}</b><span>{x[1]}</span><button>履歴</button></p>)}</article><article><h2>入金予定</h2><strong className="pf-big-number">¥1,186,000</strong><p>次回入金予定日　2026/09/10</p><button>支援者一覧をエクスポート</button></article></div>}{tab==='analytics'&&<><div className="pf-metrics">{[['閲覧数','18,736','+12.4%'],['保存数','1,256','+8.3%'],['購入数','312','+15.7%'],['転換率','1.66%','+0.23pt']].map(x=><article key={x[0]}><small>{x[0]}</small><strong>{x[1]}</strong><span>{x[2]}</span></article>)}</div><div className="pf-analytics-chart"><h2>閲覧・保存・購入の推移</h2><div>{[30,44,38,62,48,75,59,83,67,92,74,88].map((x,i)=><i key={i} style={{height:`${x}%`}}/>)}</div></div></>}{tab==='settings'&&<div className="pf-settings">{[['掲載ステータス','作品・プロフィールを一般公開',published,()=>setPublished(!published)],['おすすめ表示','トップ・特集への推薦を許可',recommended,()=>setRecommended(!recommended)],['支援受付','単発・月額支援を公開',true,()=>notice('支援受付設定を変更しました。')],['法人相談','企業からの問い合わせを受け付ける',true,()=>notice('法人相談設定を変更しました。')]].map(x=><article key={x[0] as string}><div><b>{x[0] as string}</b><p>{x[1] as string}</p></div><button className={x[2]?'on':''} onClick={x[3] as ()=>void}><span/></button></article>)}</div>}</section></main>}
...
function About({go}:{go:(v:View)=>void}){return <><Top title="ABOUT" back go={go}/><main className="content pass-page about-page">
 ...
 <section><h2>このサイトがすること</h2><div className="about-pillars">
  ...
  <article><small>02 / WALK & TICKET</small><h3>歩ける動線で巡る</h3><p>徒歩約3kmの動線をルートとして示し、デジタルチケット(構想デモ)でまとめて巡れるようにします。</p><button onClick={()=>go('pass')}>アートチケットを見る　›</button></article>
...
/* ---------- ART TICKET バナー・ページ ---------- */
function TicketQr(){return <div className="ticket-qr" aria-hidden="true"><i/><i/><i/><span/></div>}
function PassBanner({go}:{go:(v:View)=>void}){return <button className="pass-banner" onClick={()=>go('pass')}><div className="pass-banner-copy"><small>GIFU ART TICKET / DIGITAL CITY PASS</small><h2>美術館も、ギャラリーも、工芸の店も。<br/>一枚の券で、街の物語を巡る。</h2><p>岐阜公園〜川原町〜司町〜柳ヶ瀬〜駅。アートの動線をつなぐ、スマートフォンのコレクタブルチケット。</p><span>券面とプランを見る　›</span></div><aside><div className="pass-card-mini"><div className="ticket-mini-main"><small>GIFU ART & CRAFT</small><b>ART TICKET</b><em>1 DAY · GIFU CITY</em><span>№ GAT 0001</span></div><div className="ticket-mini-stub"><TicketQr/><small>ADMIT ONE</small></div></div></aside></button>}
function PassPage({go,set}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void}){return <main className="content pass-page"><section className="pass-hero" style={{backgroundImage:"linear-gradient(90deg,#061827f2 0%,#061827c4 48%,#0618273d),url('/art-ticket-gifu.png')"}}><div className="pass-hero-copy"><small>GIFU ART TICKET / DIGITAL CITY PASS</small><h1>岐阜のアートを、<br/>一枚の券で巡る。</h1><p>岐阜公園の美術館から、川原町の工芸、柳ヶ瀬の作家スペースまで。街歩きそのものを一枚の記念品にする、デジタルチケットの構想です。</p><div className="pass-hero-meta"><span>01　入館</span><span>02　街歩き</span><span>03　作り手支援</span></div><DemoNote>大学プロジェクトの構想デモです。実際の販売・決済は行われません。</DemoNote></div></section>
 <section className="pass-intro"><small>CHOOSE YOUR TICKET</small><h2>旅の長さで、券を選ぶ。</h2><p>購入後はスマートフォンに保存。受付で券面を提示し、街に点在するアートスポットを巡ります。</p></section>
 <section className="pass-plans"><div>{passPlans.map((p,i)=><article key={p.id} className={i===1?'featured':''}><div className="pass-plan-main"><small>{String(i+1).padStart(2,'0')} / {p.name}</small><strong>{p.price}</strong><b>{p.period}</b><p>{p.copy}</p><button onClick={()=>{set({passPlan:p.id});go('passcheckout')}}>この券を選ぶ　›</button></div><aside><span>{i===0?'ONE':i===1?'TWO':'PLUS'}</span><i>G<br/>I<br/>F<br/>U</i><em>ADMIT</em></aside></article>)}</div><p className="pass-note">料金・対象施設は構想段階の想定例です。各施設との提携・合意を示すものではありません。</p></section>
...
function PassCheckout({go,s,set}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void}){const uid=useId();const [plan,setPlan]=useState(s.passPlan||'1day');const sel=passPlans.find(p=>p.id===plan)!;return <main className="content pass-page"><span className="eyebrow">GIFU ART TICKET / DEMO</span><h1 className="page-title">チケット購入手続き（デモ）</h1><DemoNote>UI確認用のデモです。決済は発生せず、入力情報も送信されません。</DemoNote>
 <section className="pass-checkout-card"><h2>1. チケットを選ぶ</h2>{passPlans.map(p=><label className="pass-radio" key={p.id}><input type="radio" name={uid+'plan'} checked={plan===p.id} onChange={()=>setPlan(p.id)}/><div><b>{p.name}　{p.price}</b><small>{p.period}</small></div></label>)}</section>
 <section className="pass-checkout-card"><h2>2. お支払い方法(デモ)</h2>{['クレジットカード(デモ)','コード決済(デモ)','コンビニ支払い(デモ)'].map((x,i)=><label className="pass-radio" key={x}><input type="radio" name={uid+'pay'} defaultChecked={i===0}/><div><b>{x}</b></div></label>)}<p className="pass-note">登録・アカウント作成は不要です。チケットはこの端末に保存されます(構想)。</p></section>
 ...
 <button className="primary" onClick={()=>{set({passOwned:true,passPlan:plan});go('passdone')}}>デモ購入を確定する</button><button className="secondary" onClick={()=>go('pass')}>チケットの説明に戻る</button></main>}
function PassDone({go,s}:{go:(v:View)=>void;s:AppState}){const sel=passPlans.find(p=>p.id===s.passPlan)||passPlans[0];return <main className="content pass-page pass-done"><span className="eyebrow">GIFU ART TICKET / DEMO</span><h1 className="page-title">旅の一枚が発行されました。</h1>
 <section className="pass-card-big"><div className="pass-card-visual"><small>GIFU ART & CRAFT</small><span>DIGITAL CITY PASS</span><h2>{sel.name}</h2><b>{sel.period}</b><p>GIFU PARK · KAWARAMACHI · TSUKASA · YANAGASE</p><em>№ GAT–DEMO–0001</em></div><aside><TicketQr/><b>ADMIT ONE</b><small>受付でこの画面を提示</small></aside></section>
 <DemoNote>これはUI確認用のデモチケットです。実際の入館には使えません。</DemoNote>
 <section className="pass-list"><h2>このチケットで巡れる場所(構想中の例)</h2>{passTargets.map(x=><a key={x[0]} href={x[2]} target="_blank" rel="noreferrer"><b>{x[0]}</b><small>{x[1]}</small><span>公式 ↗</span></a>)}</section>
 <button className="primary" onClick={()=>go('walk')}>チケットで巡るルートを見る</button><button className="secondary" onClick={()=>go('home')}>ホームへ戻る</button></main>}
...
function PcExperienceList({go,set}:{go:(v:View)=>void;set:(p:Partial<AppState>)=>void}){
 ...
 return <main className="pc-page exp-index">
  ...
  <section className="exp-season"><div><small>GIFU ART TICKET</small><h2>まとめて巡るなら、1枚のチケットで。</h2><p>美術館・ギャラリー・工芸の店を1枚で巡るデジタルチケット(構想デモ)。</p><button onClick={()=>go('pass')}>アートチケットを見る　›</button></div><div>{[homeImages.park,homeImages.machiya,homeImages.souko,homeImages.mediacosmos].map((x,i)=><img key={x} src={x} alt={['岐阜公園','川原町','柳ヶ瀬','司町'][i]}/>)}</div></section>
 </main>
}
...
function DesktopMenu({go,close}:{go:(v:View)=>void;close:()=>void}){const groups:[string,[View,string,string][]][]=[['利用者向け',[['market','WORKS','作品を買う'],['artists','ARTISTS','作家を探す・フォロー'],['experience','EXPERIENCE','工房見学・制作体験'],['pass','ART TICKET','アートチケット'],['support','PATRON','単発・月額で支援'],['corporate','BUSINESS','法人購入相談']]],['旅と地域',[['walk','WALK','アートを歩いて巡る'],['eventlist','EVENT','展示・催し'],['map','MAP','地図から探す'],['about','ABOUT','このサイトについて']]],['作家・職人向け',[['creator','CREATOR STUDIO','作品・予約・支援を管理']]]];const jump=(v:View)=>{go(v);close()};return <div className="desktop-menu-layer" role="dialog" aria-modal="true" aria-label="サイトメニュー"><button className="desktop-menu-backdrop" onClick={close} aria-label="メニューを閉じる"/><aside className="desktop-menu-panel"><header><div><span>✤</span><div><b>GIFU ART & CRAFT</b><small>サイトメニュー</small></div></div><button onClick={close} aria-label="閉じる">×</button></header><section>{groups.map(([title,items])=><div className="desktop-menu-group" key={title}><h2>{title}</h2>{items.map(([v,en,ja])=><button key={v} onClick={()=>jump(v)}><span>{ja}</span><small>{en}</small><i>›</i></button>)}</div>)}</section><footer><button onClick={()=>jump('home')}>ホームへ</button><button onClick={()=>jump('creator')}>作家管理</button></footer></aside></div>}
function DesktopNav({view,go,onMenu}:{view:View;go:(v:View)=>void;onMenu:()=>void}){const links:[View,string,string][]=[['market','作品','WORKS'],['artists','作家','ARTISTS'],['experience','体験','EXPERIENCE'],['pass','チケット','ART TICKET'],['support','支援','PATRON'],['corporate','法人','BUSINESS']];return <header className="desktop-nav"><button className="desk-brand" onClick={()=>go('home')}>GIFU ART & CRAFT</button><span className="desk-tagline">岐阜の表現を、買う・体験する・支える</span><button className="desk-search" onClick={()=>go('market')}>作品・作家・体験を検索　⌕</button><nav>{links.map(([v,ja,en])=><button key={v} className={view===v?'selected':''} onClick={()=>go(v)}><span>{ja}</span><small>{en}</small></button>)}</nav><div className="desk-tools"><button className="desk-pass" onClick={()=>go('creator')}>▣<small>作家管理</small></button><button onClick={onMenu} aria-haspopup="dialog">☰<small>メニュー</small></button></div></header>}
...
function DesktopReference({view,go,s,set,notice}:{view:View;go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){
 if(view==='home')return <PlatformHome go={go} s={s} set={set} notice={notice}/>;
 if(view==='market')return <MarketPage go={go} set={set}/>;
 if(view==='artists')return <ArtistsPage go={go} s={s} set={set} notice={notice}/>;
 if(view==='artwork')return <ArtworkPage go={go} s={s} set={set} notice={notice}/>;
 if(view==='support')return <SupportPage go={go} s={s} set={set} notice={notice}/>;
 if(view==='corporate')return <CorporatePage notice={notice}/>;
 if(view==='creator')return <CreatorCenter notice={notice}/>;
...
function Screen({view,go,s,set,notice}:{view:View;go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){const p={home:<PlatformHome go={go} s={s} set={set} notice={notice}/>,market:<MarketPage go={go} set={set}/>,artists:<ArtistsPage go={go} s={s} set={set} notice={notice}/>,artwork:<ArtworkPage go={go} s={s} set={set} notice={notice}/>,support:<SupportPage go={go} s={s} set={set} notice={notice}/>,corporate:<CorporatePage notice={notice}/>,creator:<CreatorCenter notice={notice}/>,about:<About go={go}/>,pass:<PassPage go={go} s={s} set={set}/>,passcheckout:<PassCheckout go={go} s={s} set={set}/>,passdone:<PassDone go={go} s={s}/>,eventlist:<EventList go={go}/>,walk:<Walk go={go} notice={notice}/>,culture:<Culture go={go}/>,experience:<MobileExperienceList go={go} set={set}/>,shop:<Shop go={go} set={set}/>,shoplist:<ShopList go={go} set={set}/>,maker:<Maker go={go} s={s}/>,spot:<Spot go={go} s={s} set={set} notice={notice}/>,map:<MapView go={go}/>,search:<Search go={go}/>}[view];return <div className={`view view-${view}`}>{p}<Nav view={view} go={go}/></div>}
const ALL_VIEWS:View[]=['home','market','artists','artwork','support','corporate','creator','about','pass','passcheckout','passdone','eventlist','walk','culture','experience','shop','shoplist','maker','spot','map','search'];
export default function App(){const [view,setView]=useState<View>(()=>{if(typeof window==='undefined')return 'home';const h=window.location.hash.replace('#','') as View;return h&&ALL_VIEWS.includes(h)?h:'home'});const [menuOpen,setMenuOpen]=useState(false);const [toast,setToast]=useState('');const [s,setS]=useState<AppState>({saved:false,booked:false,passOwned:false,passPlan:'1day',makerId:'casa',artworkId:'wagasa-black-yellow',cart:0,following:[],supportAmount:3000});const set=(p:Partial<AppState>)=>setS(x=>({...x,...p}));const notice=(t:string)=>{setToast(t);setTimeout(()=>setToast(''),3400)};const go=(v:View)=>{setView(v);try{history.replaceState(null,'','#'+v)}catch{}};return <><main className="desktop-app"><DesktopNav view={view} go={go} onMenu={()=>setMenuOpen(true)}/><section className="desktop-screen"><DesktopReference view={view} go={go} s={s} set={set} notice={notice}/></section>{menuOpen&&<DesktopMenu go={go} close={()=>setMenuOpen(false)}/>}</main><main className="mobile-app"><Screen view={view} go={go} s={s} set={set} notice={notice}/></main>{toast&&<div className="toast">{toast}</div>}</>}
