...
type CreatorProfile={id:string;name:string;field:string;area:string;bio:string;image:string;cover:string;followers:number;kind:'art'|'craft'|'performance'};
const creatorProfiles:CreatorProfile[]=[
 {id:'aoi',name:'水野 葵',field:'現代美術・インスタレーション',area:'岐阜市・柳ケ瀬',bio:'空き店舗や路地の光を素材に、場所の記憶を組み替える。',image:'https://images.unsplash.com/photo-1494790108377-be9c29b29330?auto=format&fit=crop&w=600&q=82',cover:'https://images.unsplash.com/photo-1561214115-f2f134cc4912?auto=format&fit=crop&w=1200&q=84',followers:842,kind:'art'},
 {id:'ren',name:'河合 蓮',field:'写真・映像',area:'岐阜市・問屋町',bio:'まちの余白と人の営みを、写真と短編映像で記録する。',image:'https://images.unsplash.com/photo-1500648767791-00dcc994a43e?auto=format&fit=crop&w=600&q=82',cover:homeImages.wallart,followers:1260,kind:'art'},
 {id:'mika',name:'白木 美佳',field:'絵画・イラストレーション',area:'岐阜市・金華',bio:'長良川の水面と都市の色を、絵具と紙の層に重ねる。',image:'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=600&q=82',cover:'https://images.unsplash.com/photo-1547891654-e66ed7ebb968?auto=format&fit=crop&w=1200&q=84',followers:615,kind:'art'},
 {id:'oto',name:'長良川音景室',field:'音楽・サウンドアート',area:'岐阜市・長良',bio:'川音、まちの声、楽器を編み、岐阜の現在を音にする。',image:'https://images.unsplash.com/photo-1506794778202-cad84cf45f1d?auto=format&fit=crop&w=600&q=82',cover:homeImages.starSax,followers:934,kind:'performance'},
 {id:'casa',name:'和傘CASA',field:'岐阜和傘・工芸',area:'岐阜市・川原町',bio:'岐阜和傘を未来へつなぐ専門店。作品と職人の物語を届ける。',image:homeImages.wagasa,cover:homeImages.wagasa,followers:1710,kind:'craft'},
 {id:'souko',name:'やながせ倉庫の作家たち',field:'複合アート・クラフト',area:'岐阜市・柳ケ瀬',bio:'小さな部屋ごとに異なる表現が集まる、作家の団地。',image:homeImages.souko,cover:homeImages.soukoBoard,followers:2284,kind:'art'}
];
type Artwork={id:string;title:string;creatorId:string;category:string;price:string;stock:number;image:string;edition:string};
const artworks:Artwork[]=[
 {id:'light-river',title:'光の川 #03',creatorId:'aoi',category:'現代美術',price:'¥48,000',stock:1,image:'https://images.unsplash.com/photo-1561214115-f2f134cc4912?auto=format&fit=crop&w=900&q=84',edition:'一点もの'},
 {id:'tonya-blue',title:'問屋町、青の余白',creatorId:'ren',category:'写真',price:'¥18,700',stock:5,image:homeImages.wallart,edition:'限定20部'},
 {id:'nagara-layer',title:'NAGARA LAYERS',creatorId:'mika',category:'絵画',price:'¥66,000',stock:1,image:'https://images.unsplash.com/photo-1547891654-e66ed7ebb968?auto=format&fit=crop&w=900&q=84',edition:'原画'},
 {id:'river-sound',title:'FIELD RECORDING / 長良川',creatorId:'oto',category:'音楽',price:'¥2,200',stock:40,image:homeImages.starSax,edition:'音源＋冊子'},
 {id:'wagasa-mini',title:'岐阜和傘 小蛇の目',creatorId:'casa',category:'工芸',price:'公式価格',stock:3,image:homeImages.wagasa,edition:'受注制作'},
 {id:'souko-zine',title:'やながせ倉庫 ZINE 2026',creatorId:'souko',category:'出版・ZINE',price:'¥1,650',stock:18,image:homeImages.soukoBoard,edition:'初版100部'}
];
...
const creatorExperiences=[
 {title:'路地の光を集める写真散歩',creator:'河合 蓮',date:'9/12（土）14:00',price:'¥4,400',image:homeImages.wallart},
 {title:'作家とつくる小さな展示',creator:'水野 葵',date:'9/20（日）11:00',price:'¥6,600',image:'https://images.unsplash.com/photo-1561214115-f2f134cc4912?auto=format&fit=crop&w=800&q=82'},
 {title:'長良川フィールドレコーディング',creator:'長良川音景室',date:'10/3（土）16:30',price:'¥5,500',image:homeImages.starSax},
 {title:'岐阜和傘の仕事場を訪ねる',creator:'和傘CASA',date:'予約枠を確認',price:'公式料金',image:homeImages.wagasa}
];
...
function PlatformHome({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){return <main className="pf-page pf-home">
 ...
 <section className="pf-section"><header><small>NEW & SELECTED</small><h2>新しく届いた作品</h2><button onClick={()=>go('market')}>すべて見る　›</button></header><div className="pf-work-grid">{artworks.map(x=><ArtworkCard key={x.id} item={x} go={go} set={set}/>)}</div></section>
 ...
 <section className="pf-section pf-exp"><header><small>MEET THE MAKERS</small><h2>工房見学・制作体験</h2><p>完成品だけでなく、手と場所に出会う。</p><button onClick={()=>go('experience')}>体験一覧　›</button></header><div>{creatorExperiences.map(x=><button key={x.title} onClick={()=>go('experience')}><img src={x.image} alt=""/><span><small>{x.creator}</small><b>{x.title}</b><em>{x.date}　{x.price}</em></span></button>)}</div></section>
 ...
 <footer className="pf-footer"><b>GIFU ART & CRAFT</b><p>岐阜の表現を、買う・体験する・支える。</p><span>掲載作品・作家・価格の一部は機能確認用のデモです。</span></footer>
 </main>}

function MarketPage({go,set}:{go:(v:View)=>void;set:(p:Partial<AppState>)=>void}){const [field,setField]=useState('すべて');const [q,setQ]=useState('');const filtered=artworks.filter(x=>(field==='すべて'||x.category.includes(field))&&(x.title+creatorProfiles.find(c=>c.id===x.creatorId)?.name).includes(q));return <main className="pf-page pf-list-page"><header><small>WORKS</small><h1>作品を探す</h1><p>現代美術から工芸、写真、音楽、出版まで。</p></header><div className="pf-search"><input value={q} onChange={e=>setQ(e.target.value)} placeholder="作品名・作家名で検索"/><div>{['すべて','現代美術','写真','絵画','音楽','工芸','出版'].map(x=><button className={field===x?'active':''} key={x} onClick={()=>setField(x)}>{x}</button>)}</div></div><section className="pf-market-layout"><aside><b>絞り込み</b><label>地域<select><option>岐阜市</option><option>岐阜県全域</option><option>ほかの地域</option></select></label><label>販売形式<select><option>すべて</option><option>一点もの</option><option>受注制作</option><option>デジタル作品</option></select></label><label>価格帯<select><option>指定なし</option><option>〜¥10,000</option><option>¥10,000〜¥50,000</option><option>¥50,000〜</option></select></label></aside><div className="pf-work-grid">{filtered.map(x=><ArtworkCard key={x.id} item={x} go={go} set={set}/>)}</div></section></main>}
function ArtistsPage({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){return <main className="pf-page pf-list-page"><header><small>ARTISTS & MAKERS</small><h1>作家・工房を探す</h1><p>分野、地域、技法から。別地域で活動する同じ作家も追いかけられます。</p></header><section className="pf-artist-list">{creatorProfiles.map(c=><article key={c.id}><img src={c.cover} alt=""/><div><small>{c.field}</small><h2>{c.name}</h2><p>{c.bio}</p><span>⌖ {c.area}　♡ {c.followers.toLocaleString()}</span><div><FollowButton id={c.id} s={s} set={set} notice={notice}/><button onClick={()=>{const work=artworks.find(x=>x.creatorId===c.id);if(work){set({artworkId:work.id});go('artwork')}else go('support')}}>作品・支援を見る</button></div></div></article>)}</section></main>}
function ArtworkPage({go,s,set,notice}:{go:(v:View)=>void;s:AppState;set:(p:Partial<AppState>)=>void;notice:(t:string)=>void}){const item=artworks.find(x=>x.id===s.artworkId)||artworks[0];const c=creatorProfiles.find(x=>x.id===item.creatorId)!;return <main className="pf-page pf-detail"><button className="pf-back" onClick={()=>go('market')}>‹ 作品一覧</button><section><div className="pf-detail-image"><img src={item.image} alt=""/><span>{item.category}</span></div><article><small>{c.field} / {c.area}</small><h1>{item.title}</h1><button className="pf-artist-link" onClick={()=>go('artists')}><img src={c.image} alt=""/><span><b>{c.name}</b><small>{c.bio}</small></span></button><strong>{item.price}</strong><p>作品証明書付き。作品の背景、素材、設置方法を確認してから購入できます。</p><dl><div><dt>販売形式</dt><dd>{item.edition}</dd></div><div><dt>在庫</dt><dd>{item.stock}点</dd></div><div><dt>配送</dt><dd>作家・工房から直送</dd></div></dl><button className="pf-buy" onClick={()=>{set({cart:s.cart+1});notice('カートに追加しました。')}}>カートに入れる（{s.cart}）</button><button className="pf-reserve" onClick={()=>go('corporate')}>法人・空間導入を相談</button><FollowButton id={c.id} s={s} set={set} notice={notice}/></article></section><section className="pf-detail-story"><div><small>STORY</small><h2>作品の先にある、場所と時間。</h2><p>{c.bio} 本作品の購入代金は、決済手数料等を除き作家へ支払われます。購入後も作家をフォローし、新作・展示・体験のお知らせを受け取れます。</p></div><aside><h3>この作家を支援する</h3><p>作品購入とは別に、次の制作や展示を応援できます。</p><button onClick={()=>{set({makerId:c.id});go('support')}}>単発・月額支援を見る</button></aside></section><section className="pf-related"><header><h2>別地域でも、この作家を探す</h2><p>展示・取扱店・体験会を地域横断で表示。</p></header>{['岐阜市・柳ケ瀬','名古屋・栄','東京・清澄白河'].map((x,i)=><article key={x}><span>{i===0?'開催中':'次回予定'}</span><b>{x}</b><small>{i===0?'アトリエ公開':'グループ展・取扱店'}</small></article>)}</section></main>}
